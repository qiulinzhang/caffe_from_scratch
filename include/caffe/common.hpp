...
// Conver macro to string
#define STRINGIFY(m) #m \\ '#' 构川操作符
#define AS_STRING(m) STRINGIFY(m) \\ 宏遇到宏不会展开，仍然是原来的字母
...
private:\
    ...
    classname& operator=(const classname&) // 重写赋值操作符
...
#define INSTANTIATE_LAYER_GPU_BACKWARD(classname) \
    template void classname<float>::Backward_gpu(\
        const std::vector<Blob<float>*>& top, \
        const std::vector<Blob<bool>*>& propagagte_down, \
        const std::vector<Blob<float>*>& bottom); \
...
namespace caffe{
    ...
    void GlobalInit(int* pargc, char*** pargv); // 三个***的意义 ????
    ...
    class Caffe{
        public:
            ...
            class RNG{
                public:
                    RNG();
                    explicit RNG(unsigned int seet);
            ...
            static void set_random_seed(const unsigned int seet);
            ...
            inline static void set-solver_count(int val) {Get().solver_count_=val;}
            inline static int solver_rank() {return Get().solver_rank_;}
            inline static void set_solver_rank {Get().solver_rank_ = val;}

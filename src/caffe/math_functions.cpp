...
// 主要是需要的数学运算，比如 加减乘除，指数 log, ax+by , gemm矩阵乘法等
namespace caffe {

    template <>
    void caffe_cpu_gemm<float>(const CBLAS_TRNANSPOSE TransA, Const CBLAS_TRNANSPOSE TransB,
                        const int M, const int N, const int K,
                        const float alpha, const float* A, const float* B, const float beta,
                        float* C) {
        // 矩阵A: MxK，矩阵B: KxN，矩阵C: MxN
        // C := alpha*op(A)*op(B) + beta*C，其中 op(A)是根据 TransA 这个标志位来判断 A 是否转置
        // 采用行major的方式，即对于矩阵A，MxK, M是行的数量，N是列的数量
    ...
    template <>
    void caffe_cpu_gemm<double>(const CBLAS_TRNANSPOSE TransA, Const CBLAS_TRNANSPOSE TransB,
                        const int M, const int N, const int K,
                        const double alpha, const double* A, const double* B, const double beta,
                        double* C) {
        // 矩阵A: MxK，矩阵B: KxN，矩阵C: MxN
        // C := alpha*op(A)*op(B) + beta*C，其中 op(A)是根据 TransA 这个标志位来判断 A 是否转置
        // 采用行主序的方式，即同一行的元素在内存中是相邻的
        int lda = (TransA == CblasNoTrans) ? K : M;
        int ldb = (TransB == CblasNoTrans) ? N : K;
    ...
    template <>
    void caffe_cpu_gemv<float>(const CBLAS_TRNANSPOSE TransA, const int M, const int N,
                        const float alpha, const float* A, const float* x, const float beta,
                        float* y) {
        // 矩阵和向量的乘法 // y:=alpha*op(A)*x + beta*y
        cblas_sgemv(CblasRowMajor, TransA, M, N, alpha, A, N, x, 1, beta, y, 1);
                        }

    template <>
    void caffe_cpu_gemv<double>(const CBLAS_TRNANSPOSE TransA, const int M, const int N,
    ...
    template<>
    void caffe_set<int>(const int N, const int alpha, int* Y);
    ...
    template<>
    void caffe_copy<unsigned int>(const int N, const unsigned int* X, unsigned int* Y);
    ...
    template<>
    void caffe_add_scalar(const int N, const double alpha, double* X) {
        for (int i=0; i < N; ++i) {
            X[i] += alpha;
        }
    }
